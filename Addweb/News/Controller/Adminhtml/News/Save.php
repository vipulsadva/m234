...
class Save extends \Magento\Backend\App\Action
{
	...
	public function execute()
    {
        ...
        if ($data) {
            $model = $this->_objectManager->create('Addweb\News\Model\Custom');
		    ...
		    if(@$data['image']['delete']){
             ...
             if(isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
				try {
						...
						$result = $uploader->save($mediaDirectory->getAbsolutePath('newsimages'));
						unset($result['tmp_name']);
						unset($result['path']);
						$data['image'] = 'newsimages'.$result['file'];
				} catch (Exception $e) {
